...
export async function generateStoryScenes(topic: string, count: number): Promise<Scene[]> {
    console.log(`[Backend] Generating ${count} scenes for topic: "${topic}"...`);
    const userPrompt = `Generate a short story about "${topic}". Break the story down into exactly ${count} distinct scenes. For each scene, provide a descriptive background, a detailed character description (e.g., "A wise old wizard with a long white beard and starry robes"), and the scene text. If no character is present, the character should be null. Ensure the output is a valid JSON array.`;
            ...
            responseSchema: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        "background": { "type": "STRING", "description": "A descriptive filename for a background image, e.g., 'haunted_castle.jpg'" },
                        "character": { "type": "STRING", "description": "A detailed description of the character in the scene, or null." },
                        "text": { "type": "STRING", "description": "The narrative text for this scene." }
                    },
...
export async function generateCharacterImage(characterDescription: string): Promise<string | null> {
