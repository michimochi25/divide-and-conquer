...
export async function generateQuestions(textData: string, count: number): Promise<Question[]> {
    console.log(`[Backend] Generating ${count} questions for the provided text...`);

    const userPrompt = `Generate exactly ${count} distinct multiple-choice questions from the following text. For each question, provide 4 options and ensure the correct answer is one of those options
    Text:
    """
    ${textData}
    """`;
